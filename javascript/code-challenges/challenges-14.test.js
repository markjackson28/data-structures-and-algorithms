...
const screenForNames = (arr) => {
  let regex = /^(Mr\.|Mrs\.|Ms\.|Dr\.).[a-zA-Z0-9_]*$/gm;
...
const toTitleCase = (arr) => {
  return arr.map(lilWord => lilWord.charAt(0).toUpperCase() + lilWord.slice(1));
};
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(character => {
    if (character.mass > 77) {
      biggerThanLuke.push(character.name);
    }
  });
  return biggerThanLuke.join(' - ');
};
...
//does not sort by name; only by number
